...
| **Extension (ORG-003, F-026–029):** one new optional per-option prop — **`active?: boolean`** — marks the currently-selected choice in a single-select menu (the sort menu's active sort key). Backward-compatible; every existing call site (photo menu, row/Detail action menus, delete-confirm) omits it and renders unchanged. Mirrors `destructive`'s existing color+weight-pairing mechanism exactly: a third resolved color, `activeIconColor = rgbFromTriplet(NATIVE_CHROME_RGB[resolvedScheme].primary)` (alongside the existing `default`/`destructive` resolutions — `primary` is already a key on `NATIVE_CHROME_RGB`, no new resolution pattern), and the label gains `font-semibold text-primary` in place of `text-text` — never color-alone (anti-pattern #3). The option's own leading `icon` is **not** swapped to a checkmark — it keeps its meaningful glyph (e.g. the sort menu's `calendar`/`clock`/`type`/`refresh-cw`), only its color+weight change. **No new tokens** — `primary` and `font-semibold` are both already budgeted. See the ORG-003 "Sort control" subsection for the full menu spec. |
...
### ORG-003 — Sort control (F-026–029)

**Fills ORG-001's reserved slot 3** — the sort trigger becomes the **second child** of the existing `flex-row items-center gap-3` row (slot 2 + 3), immediately after the filter's `flex-1` wrapper. No change to slot 1 (ORG-002's search row) or the filter `SegmentedControl` itself — this is the **last** ORG-001 reservation to fill; the organize header is structurally complete after this task.

```
<View className="flex-row items-center gap-3">      {/* slot 2 + 3 — unchanged container, ORG-001 */}
  <View className="flex-1">
    <SegmentedControl ... />                          {/* slot 2 — ORG-001, unchanged */}
  </View>
  <IconButton
    icon="sliders"
...
**Sort trigger** — the `IconButton` primitive (`@/components/ui`) verbatim, as the row's second child: Feather **`sliders`**, the component's own default `iconSize` (20dp) and `min-h-12 min-w-12` (48dp) chrome, unmodified — no `className` override needed. Color: **neutral `text-muted`** (`iconColor={rgbFromTriplet(NATIVE_CHROME_RGB[resolvedScheme].textMuted)}`, the same `mutedColor` resolution ORG-002's "Clear search" `IconButton` already uses) — this is an **overflow-style trigger that opens a menu, not a state signal itself**, the exact reasoning TSK-004's Detail "More" button already established ("a neutral overflow trigger, not a state signal") — this is that pattern's second application. Deliberately **not** tinted `primary` for a non-default sort — that would read as a second, competing accent next to the Signal-Teal filter-track/FAB accents already on this screen, and isn't required: the active sort is communicated inside the menu (below), not on the trigger's chrome.

`accessibilityLabel` is **dynamic**, always announcing the current sort — `` `Sort tasks, currently ${SORT_LABELS[sort]}` `` (e.g. "Sort tasks, currently Creation date" on first launch). Introduce one small shared label map, **`SORT_LABELS: Record<TaskSortKey, string>`** (frontend — `features/tasks/`), reused for BOTH this label and the menu's own option labels below, so the trigger and the menu can never drift into two different names for the same key — the same "one canonical string, two consumers" discipline `formatDueDate.ts` already established for date formatting.

**Row fit:** the row itself is unchanged from ORG-001 (`flex-row items-center gap-3`) — the filter's own `flex-1` wrapper already cedes exactly the width the 48dp `IconButton` needs; no width/height change to the `SegmentedControl` track. The 56dp track vs. 48dp button height mismatch is the **same accepted asymmetry ORG-001's own height-budget note already priced in** ("a multi-option track reading slightly taller than a single icon action... not something to force-align later") — nothing new to reconcile.

**Sort `ActionSheet` menu (FR2)** — opens via the trigger's `onPress`; `HomeScreen`-owned `sortSheetVisible` boolean (the same "one sheet, one piece of local state" ownership pattern the row-actions/delete-confirm sheets in TSK-004 already use):

- `title="Sort by"`
- `accessibilityLabel="Sort by"` (matches the visible header — the same "accessible name matches its visible header" convention the delete-confirm sheet already established).
- `options`, in this order:

| # | label | `TaskSortKey` | icon | `onPress` |
|---|---|---|---|---|
| 1 | "Due date" | `due` | `calendar` | `taskQueryStore.setSort('due')` |
| 2 | "Creation date" | `created-desc` | `clock` | `taskQueryStore.setSort('created-desc')` |
| 3 | "Alphabetical" | `alpha` | `type` | `taskQueryStore.setSort('alpha')` |
| 4 | "Recently updated" | `updated` | `refresh-cw` | `taskQueryStore.setSort('updated')` |

Icon choices: `calendar` (due date — reuses the **exact** glyph already on `TaskListItem`/`TaskDetailScreen`/`DueDateField`'s own due-date rows, not a fresh association), `clock` (creation — a plain time glyph, deliberately distinct from `calendar` so the two date-flavored options don't share a silhouette), `type` (alphabetical — the closest Feather glyph to "sort by name/text"), `refresh-cw` (recently updated — a "changed" glyph, distinct from the other three). None of these four is reused elsewhere in this menu, so every option stays distinguishable at a glance, not only by label text. `ActionSheet`'s own option `onPress` wrapper already calls `onClose()` before firing the option's `onPress` — selecting a sort closes the sheet with no extra code.

**Active-sort indication — an `ActionSheet` extension, not a token.** The primitive has no per-option "selected" concept today, only `destructive`. Extend `ActionSheetOption` with one new optional field, **`active?: boolean`**, mirroring `destructive`'s existing mechanism exactly (full extension spec + rationale in the `ActionSheet` Component-inventory entry above — backward-compatible, every other existing call site renders unchanged). Effect on the currently-selected sort's row only:
- **icon color** → `NATIVE_CHROME_RGB[resolvedScheme].primary` (a third resolved color alongside the existing `default`/`destructive` ones) — the option **keeps its own meaningful glyph** (its `calendar`/`clock`/`type`/`refresh-cw`, never swapped to a generic check).
- **label** → `font-semibold text-primary` in place of `text-text`.
- Both change together — never color-alone (anti-pattern #3), the identical two-property pairing `destructive` already uses for its own icon+label.

`HomeScreen`'s call site builds the option list with `active: key === sort` per option (derived from the store on every render — the same "derive from state, don't stash a duplicate boolean" pattern the row menu's own dynamic "Mark complete"/"Mark pending" label already uses). Reusing `primary` here is the same token already documented for "focus ring"/"active tab" — a selected menu row is the same "currently active choice" concept `SegmentedControl`'s own selected-segment fill already expresses, just via icon+text color+weight instead of a track fill (the row shape doesn't support a solid selected-fill without visually breaking from every other `ActionSheet` menu in the app).

**Copy + labels** (Maestro-anchorable on the visible option text, the same convention every other `ActionSheet` menu in this app already follows):

| Element | String |
|---|---|
| Trigger `accessibilityLabel` | `` `Sort tasks, currently ${SORT_LABELS[sort]}` `` — dynamic, e.g. "Sort tasks, currently Creation date" |
| Sheet `title` / `accessibilityLabel` | `"Sort by"` |
| Option 1 | `"Due date"` |
| Option 2 | `"Creation date"` |
| Option 3 | `"Alphabetical"` |
| Option 4 | `"Recently updated"` |
| Built-in Cancel | `"Cancel"` (unchanged — from `ActionSheet` itself) |

**A11y-tree shape** (extends the ORG-001/ORG-002 block — insert immediately after the 3 filter tabs; the sheet's own nodes only mount while `sortSheetVisible`):
```
...
tablist: "Filter tasks"
  tab: "All"
  tab: "Active"
  tab: "Completed"
button: "Sort tasks, currently <sort label>"
(list of task rows | empty state — unchanged, ORG-001/ORG-002)
```
— while the sort sheet is open, it mounts over the tree exactly like the row/Detail/delete-confirm sheets already do:
```
header: "Sort by"
menuitem: "Due date"
menuitem: "Creation date"
menuitem: "Alphabetical"
menuitem: "Recently updated"
button: "Cancel"
```

**Tokens used:** `text-muted` (trigger icon, neutral), `primary` (active-option icon+label — its own documented "focus ring"/"active tab" usage), `text` (default/unselected option label+icon — unchanged from the existing `ActionSheet` recipe), `card`/`border`/`radius-lg` (sheet chrome — unchanged, inherited), spacing step 3 (`gap-3` — the row's existing gap, unchanged), `text-base` + the existing 400/600 weight pair. **Zero new tokens** — the one addition is a **component prop** (`ActionSheetOption.active?: boolean`), not a token or hex value; it resolves through the already-existing `NATIVE_CHROME_RGB.primary` entry, the same mechanism every other native-prop color in the app already uses.

**Missing tokens check:** none. **Verified against banned-defaults #1–8:** no pure black/white, no default-RN-blue, no color-only state (the active option pairs `primary` color with a weight change — `font-semibold` — on both the icon and the label, mirroring `destructive`'s existing two-property pairing exactly; the trigger itself carries no state color at all, per the "overflow trigger, not a state signal" reasoning above), no hardcoded hex (all three icon colors route through `NATIVE_CHROME_RGB`/`rgbFromTriplet`), no shadow additions, weight budget unchanged (options stay within the existing 400/600 pair — no third weight introduced), 4pt-grid spacing only (the row's existing `gap-3`, the sheet's existing `gap-1`/`p-4`), no new semantic color (`primary` reused for its own documented usage, not a fresh mapping). Did NOT touch `mobile/src` (authz boundary — `frontend` implements from this spec).
...
- 2026-07-12 — design agent (ORG-003 visual spec): added the "Sort control" subsection (F-026–029) — fills ORG-001's reserved slot 3, the **last** organize-header reservation (the header is structurally complete after this task). A compact `IconButton` sort trigger (Feather `sliders`, neutral `text-muted` — an overflow-style trigger opening a menu, not a state signal itself, the same reasoning as TSK-004's Detail "More" button) lands as the row's second child beside the filter `SegmentedControl`, with a dynamic `accessibilityLabel` ("Sort tasks, currently \<label\>") driven by one new shared `SORT_LABELS` map also reused by the menu's own option labels. Opens a 4-option `ActionSheet` (`title="Sort by"`): Due date/`calendar`, Creation date/`clock`, Alphabetical/`type`, Recently updated/`refresh-cw` — each icon distinct, none reused elsewhere in the menu. **One small, backward-compatible `ActionSheet` extension** (documented in the `ActionSheet` Component-inventory entry above): `options[].active?: boolean`, mirroring the existing `destructive` mechanism exactly — the active sort's own icon (never swapped to a checkmark) plus its label render in `primary` + `font-semibold`, never color-alone, resolved through the already-existing `NATIVE_CHROME_RGB.primary` entry (a third resolved color alongside `default`/`destructive`). **Zero new tokens** — reuses `primary`/`text`/`text-muted`, the existing 400/600 weight pair, and the row's existing `gap-3`; the one addition is a component prop, not a token. Verified against banned-defaults #1–#8: no pure black/white, no default-RN-blue, no color-only state, no hardcoded hex, no shadow additions, weight budget unchanged, 4pt-grid spacing only, no new semantic color. Did NOT touch `mobile/src` (authz boundary — `frontend` implements from this spec).
