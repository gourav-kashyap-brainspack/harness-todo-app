...
| f | Write-time throws are unswallowed by contract | ✅ **RESOLVED for PRO (PRO-001, PR #10); still open for TSK** | PRO-001's form validates via `zodResolver(profileSchema)` at the boundary — `saveProfile`'s throw-on-invalid path is unreachable from `ProfileSetupScreen`, see patterns-registry.md → "RHF + Zod form pattern". TSK must still apply the same pattern to `saveTasks`/`upsertTask` when it builds its forms. PRO-003 still owns the `photo` image-picker/permission flow (STG persists only the file-uri string), not yet built. |
