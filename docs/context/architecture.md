...
## ORG — module started (2026-07-12, ORG-001 review)
...
boundary violation). **ORG-001 (the anchor, F-023/024/025) is all-gates-green — PR #18, `status: review`,
NOT yet merged/done.** It adds:
...
ORG-002 (search, F-020/021/022/031) and ORG-003 (sort, F-026–029) are both `blocked` on ORG-001 and share
`HomeScreen.tsx`/`taskQueryStore.ts`/`selectVisibleTasks.ts` (group `ORG-pg1`) — coordinate via
`parallel-integration` or serialize (recommended: 002 then 003). No new deps, no schema/data-model change.
See patterns-registry.md → "Derived selector over a store" / "UI-query-state persisted via the typed storage
service" for the 2 new Tier-1 rows this task established, and `docs/context/stack.md` → "Spec-gap notes
(flagged during ORG-001)" for 2 forward security advisories ORG-002 must respect (untrusted `search` → no
user-built `RegExp`, ReDoS risk).
