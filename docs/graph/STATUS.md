...
**Project:** Todo App · **Updated:** 2026-07-12 · **Phase:** FND + STG + PRO + **TSK done** (TSK-001–005 merged PRs #13–17; coherence PASS, E2E deferred, integration go). **ORG feature-complete** — `/module ORG` wrote 3 specs (ORG-001/002/003, 11 features); **ORG-001 (the anchor) done — PR #18 merged. ORG-002 (search + no-results) done — PR #19 merged.** **ORG-003 (sort — the LAST ORG task and last task overall) is `review`, PR #20 open, all gates green, awaiting human merge-go — NOT yet merged/done.** ORG is the **last module** — ORG-003's merge triggers the ORG Module DoD (coherence + local E2E) AND completes the whole 48/48-feature build; after that, only the deferred-E2E batch + the `development → main` release lane remain.
...
| 5 | **ORG** — Search, Filter, Sort | 11 | 3 (2 done, 1 review) | TSK ✅ | 🟡 **feature-complete, Module DoD pending** — ORG-001/002 done (PRs #18/#19 merged); ORG-003 review (PR #20), all gates green, awaiting human merge-go |
...
| **ORG-002** | Search (title+desc, real-time) + no-results empty state | F-020/021/022, F-031 | M | 4h | ORG-001 ✅ | ORG-pg1 | 🟢 done — PR #19 merged 2026-07-12 |
| **ORG-003** | Sort — due · created · alpha · updated (ActionSheet menu) | F-026/027/028/029 | M | 4h | ORG-001 | ORG-pg1 | 🟡 review — PR #20 open, all gates green, awaiting human merge-go — NOT yet merged/done |

**Critical path:** ORG-001 → ORG-002 → ORG-003 (serialized, ≈16h). **ORG-001** (the anchor) is 🟢 **done** — PR #18 merged 2026-07-12, all gates green (typecheck·lint·331 unit, 100% changed files·Android `assembleDebug`·security PASS, 2 Low forward advisories·code-review APPROVE). **ORG-002** (search + no-results) is 🟢 **done** — PR #19 merged, all gates green (typecheck·lint·358 unit, changed files 94–100%·Android `assembleDebug`·security PASS, both ORG-001 advisories verified resolved·code-review APPROVE, 2 optional non-blocking nits). **ORG-003** (sort — the LAST ORG task + last task overall) is 🟡 **review** — PR #20, branch `feat/ORG-ORG-003`, all gates green (typecheck·lint·376 unit, changed files 96–100%·Android `assembleDebug`·security PASS·code-review APPROVE, 2 optional non-blocking nits), **NOT yet merged**, awaiting human merge-go. Fills the LAST `selectVisibleTasks` seam (`SORT_COMPARATORS`) — the search→filter→sort pipeline is now structurally complete. Its merge triggers the **ORG Module DoD** (coherence review + local E2E) **and completes the whole 48/48-feature build. ORG total:** ≈16h.
**ORG decisions (/module ORG 2026-07-12):** search/filter/sort live **on Home** (derived selector, not a separate screen) · all three **compose** in one pipeline (search → filter → sort) · filter+sort **persist via the typed storage service** as one `{filter,sort}` envelope (`createPersistedValue`, NOT the themeStore raw-key shape — reconciled in ORG-001.spec.md post-implementation), search **in-memory** (resets on relaunch) · default sort **created-desc** (newest) · filter = `SegmentedControl` (reused), sort = `ActionSheet` menu (reused), no-results = `EmptyState` (reused) · **ORG code lives in `features/tasks`** (NOT a features/organize slice — that would be a features→features boundary violation reading the tasks store) · OQ-8 preserved (no re-sort on toggle). No new deps, no schema change, no research spike. **ORG-001 promoted 2 new Tier-1 patterns** (derived-selector-over-a-store; UI-query-state via typed storage service). **ORG-002 resolved both ORG-001 forward security advisories and promoted a 3rd Tier-1 pattern** ("Untrusted search-input handling (ReDoS-safe)"). **ORG-003 completes the derived-selector pipeline (all 3 seams filled) and extends `ActionSheet` a 3rd time** (`options[].active?` — single-select indicator) — see patterns-registry.md.

**Next:** human merge-go on PR #20 (ORG-003 — the LAST task). Its merge triggers the ORG Module DoD (coherence + local E2E), completing the whole 48/48-feature build; then the deferred-E2E batch + `development → main` release lane remain (both mandatory, both need disk freed).
