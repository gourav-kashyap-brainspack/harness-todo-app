 ...
 * through. On submit it merges the validated `title`/`description` over the
 * EXISTING task's `status`/`dueDate` and calls `updateTask` with the FULL
 * business-field set (STG gap a — `taskRepository.upsertTask` wholesale-
 * replaces on update; a partial submit would blank out `status`/`dueDate`).
