...
> **Detail moved to `docs/context/clickup-mirror.md`** (mapping · status-sync milestones · automation) — read it for `/clickup-sync` or any ClickUp mirror work; operational specifics also live in the **clickup-sync** skill + `docs/graph/modules.json`. Harness **mirrors the plan** (ClickUp reflects the harness, never the reverse): `/scope`+`/module` create Lists/Tasks, `/build` syncs status. **Resolved IDs (mirrored 2026-07-10):** Workspace `90161694196` (**Harness Learning**) · Space `90167430002` (**TODO APP**) · Lists: FND `901615807764` · STG `901615807765` · PRO `901615807766` · TSK `901615807767` · ORG `901615807768`. Full task→ClickUp-id map in `docs/graph/modules.json` (`clickup.tasks`). **Status map (this Free-plan space has only `to do`/`complete` — no `in progress`):** `started/review/in-progress → to do (+ context comment)` · `done → complete`. Merge→`complete` needs the `CLICKUP_TOKEN` repo secret.
